...
parser.add_argument('--test_dataset', type=list, default=['DUTS-TE'])
# Misc
...
def test(test_loader,model,epoch,save_path):
    ...
    with torch.no_grad():
        for i, data_batch in enumerate(test_loader):
            image,label, name, split, size = data_batch['image'],data_batch['label'], \
...
if __name__ == '__main__':
    ...
    composed_transforms_ts = transforms.Compose([
        transform_single.RandomFlip(),
        transform_single.RandomRotate(),
        transform_single.colorEnhance(),
    ...
    dataset_train = Dataset(datasets=['DUTS-TR'],transform=composed_transforms_ts, mode='train')
